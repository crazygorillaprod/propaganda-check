...
export function TeachingTakeDisplay({ teachingTake, topic, isLocked = false }: TeachingTakeDisplayProps) {
  ...
  const TAGLINE = 'Evidence-first checks on political claims and power';
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      // Title
      addText('CIVIC BREAKDOWN', 18, true);
      ...
      doc.save(`civic-breakdown-${Date.now()}.pdf`);
    } catch (err) {
  ...
  const exportToText = () => {
    ...
    const exportText = `
     CIVIC BREAKDOWN - Evidence-Based Analysis
========================================
${topic ? `\nTopic: ${topic}\n` : ''}
${summary ? `SUMMARY\n${summary}\n\n` : ''}
WHAT WE KNOW
...
`.trim();
    ...
    a.href = url;
    a.download = `civic-breakdown-${Date.now()}.txt`;
  ...
  const shareViaEmail = () => {
    const subject = encodeURIComponent(topic || 'Civic Breakdown - Evidence-Based Analysis');
    ...
    <div className="relative">
        ...
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/60 backdrop-blur-sm rounded-lg">
          <div className="bg-white p-8 rounded-xl shadow-2xl border-2 border-purple-300 max-w-md text-center">
            ...
            <p className="text-gray-600 mb-6">
              Full Civic Breakdowns with plain-language summaries, receipts-based responses, and exportable resources.
            </p>
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg p-6 border border-indigo-200">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Civic Breakdown</h2>
            <p className="text-sm text-gray-600">
              Plain-language summary + receipts-based response
            </p>
            {topic && (
